...
- **`revoke ... from public` BUKAN penjagaan di Supabase.** Supabase memberi `EXECUTE` **langsung ke peran `anon` & `authenticated`** (default privileges skema `public`), jadi fungsi baru bisa dipanggil lewat `/rest/v1/rpc/<nama>` dengan kunci anon walaupun sudah dicabut dari PUBLIC — terbukti pada `hari_ini_wib()` di 0094 (diperbaiki di 0095 dengan `revoke execute ... from anon`). Fungsi yang tak boleh dipanggil tanpa login harus dicabut dari `anon` **secara eksplisit**, dan buktikan dengan benar-benar memanggilnya memakai kunci anon — jangan percaya pada baris `revoke` yang tak diuji.
...
- Migrasi SQL di `supabase/migrations/0001..0096`, **dijalankan MANUAL berurutan** di **Supabase SQL Editor** (tidak ada CLI migrate). Saat menambah skema: buat file `00NN_*.sql` baru, lalu minta user menjalankannya.
