...
- **Optimasi gambar**: upload gambar dikompres di klien via **`lib/img.ts` `kompresGambar(file,{maksDim,kualitas})`** — downscale + encode WebP (jaga transparansi; SVG/GIF/PDF dilewati; fallback ke asli bila lebih besar/gagal). Titik kompres: AsetInput 512px, ikon tema 256px, **bukti pembayaran** (DaftarForm & BuktiUpload) 1280/0.8, dokumen (UploadDok) 1280/0.8, sampul artikel/gambar produk/banner event 1280/0.82. **DIKECUALIKAN** (butuh resolusi cetak): background **sertifikat & stiker** event. `<img>` di `Aset`/`Sampul` pakai `loading=lazy`+`decoding=async`.
