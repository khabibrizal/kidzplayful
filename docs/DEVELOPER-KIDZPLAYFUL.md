...
- **Optimasi gambar**: upload gambar (AsetInput 512px, ikon tema 256px) dikompres di klien via **`lib/img.ts` `kompresGambar()`** — downscale + encode WebP (jaga transparansi; SVG/GIF dilewati; fallback ke asli bila lebih besar/gagal) → file kecil, game ringan. `<img>` di `Aset`/`Sampul` pakai `loading=lazy` + `decoding=async`. Gambar lama tak ikut berubah (upload ulang bila perlu).
