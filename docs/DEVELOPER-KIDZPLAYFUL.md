...
- **Bagikan ke Feed (1:1)** — `lib/feed-card.ts` `buatKartuFeed()`, **1080×1080**:
  ...
  - **`gambarCover(..., fokusY)`**: kotak foto di Feed berbentuk portrait sementara sampul artikel umumnya landscape → crop tengah **memenggal wajah**. Feed memakai `fokusY = 0.3` (bidik ke atas). Ini terlihat jelas pada render percobaan pertama dan diperbaiki setelah diperiksa visual.
