// src/app/admin/keuangan/aset/page.tsx — pencatatan aset
import { getAset } from '@/lib/data/keuangan';
...
export default async function AsetPage() {
  ...
  async function aksiHapus(formData: FormData) {
    'use server';
    await hapusAset(String(formData.get('id')));
  }
    ...
    <div>
      ...
      <form action={simpanAset} className={s.card}>
        <div className={s.row} style={{ gap: 6, flexWrap: 'wrap' }}>
          ...
          <input className={s.inp} name="kategori" placeholder="Kategori" style={{ flex: 1, minWidth: 120 }} />
        </div>
        ...
        <div className={s.row} style={{ gap: 6, flexWrap: 'wrap', marginTop: 6 }}>
          <input className={s.inp} name="lokasi" placeholder="Lokasi" style={{ flex: 1, minWidth: 120 }} />
          <input className={s.inp} name="invoice_url" placeholder="URL invoice (opsional)" style={{ flex: 2, minWidth: 160 }} />
        </div>
      ...
      {list.map((a) => (
        <div key={a.id} className={s.card}>
          <div className={s.row}>
            <span style={{ flex: 1 }}><b>{a.nama}</b>{a.kategori ? ` · ${a.kategori}` : ''}<br />
              <small className={s.muted}>{formatRupiah(a.harga_beli)}{a.tanggal_beli ? ` · ${a.tanggal_beli}` : ''}{a.lokasi ? ` · ${a.lokasi}` : ''}{a.umur_manfaat_bulan ? ` · ${a.umur_manfaat_bulan} bln` : ''}</small></span>
