  ...
  return (
    <div style={{ position: 'relative', width: '100%', maxWidth: 900, margin: '0 auto', aspectRatio: '1.414 / 1', background: 'linear-gradient(135deg,#f6f1ff 0%,#eafaf1 100%)', border: '6px double var(--lavender-d, #6b4fb0)', borderRadius: 14, overflow: 'hidden' }}>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '6% 8%' }}>
        <Logo height={44} />
        <div style={{ marginTop: '3%', letterSpacing: 4, fontSize: 'clamp(12px,2.4vw,20px)', fontWeight: 800, color: 'var(--mint-d, #2e9e63)' }}>SERTIFIKAT</div>
        <div style={{ marginTop: '1.5%', fontSize: 'clamp(9px,1.5vw,13px)', color: 'var(--abu, #8a83a0)' }}>Diberikan kepada</div>
        <div style={{ marginTop: '1%', fontSize: 'clamp(16px,3.8vw,32px)', fontWeight: 800, color: 'var(--lavender-d, #6b4fb0)' }}>{s.anak_nama}</div>
        <div style={{ marginTop: '2%', fontSize: 'clamp(9px,1.7vw,15px)', color: 'var(--tinta, #3a3350)' }}>atas partisipasinya dalam kelas bermain</div>
        ...
        <div style={{ marginTop: '4%', fontSize: 'clamp(8px,1.3vw,12px)', color: 'var(--abu, #8a83a0)' }}>KidzPlayful{s.diterbitkan_oleh ? ` · ${s.diterbitkan_oleh}` : ''}</div>
      </div>
    </div>
  );
}
