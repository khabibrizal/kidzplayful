...
type RawKustom = { id: string; judul: string; lencana_kode: string; syarat: SyaratItem[] };
...
export async function getGamifikasiAnak(anakId: string): Promise<GamifikasiAnak> {
  ...
  try {
    ...
    const [{ data: anak, error: e1 }, { data: rows }, { data: lenc }, { data: kustomDef }, { data: kustomDone }] = await Promise.all([
      ...
      s.from('tantangan_kustom').select('id,judul,lencana_kode,syarat').eq('aktif', true),
    ...
    const kustom: TantanganKustomRingkas[] = ((kustomDef ?? []) as RawKustom[]).map((k) => {
