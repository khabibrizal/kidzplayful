...
export interface PengaturanTrial {
  trial_kelas: boolean;     // trial boleh akses materi kelas bermain
  trial_game: boolean;      // trial boleh akses game edukasi
  trial_video: boolean;     // trial boleh akses pojok video
...
export const DEFAULT_TRIAL: PengaturanTrial = {
  trial_kelas: true, trial_game: true, trial_video: true, trial_maks_anak: 3,
};
...
export async function getPengaturanTrial(): Promise<PengaturanTrial> {
  try {
    ...
    const { data } = await supabase
      ...
      .select('trial_kelas,trial_game,trial_video,trial_maks_anak')
      .eq('id', 1)
      .single();
