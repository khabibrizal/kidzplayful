// Verifikasi e2e Mode Tantangan (bonus cepat) + Waktu per game di Rapor. Prasyarat: migrasi 0033.
// npm run build && npm start, lalu: node tools/bonus_check.mjs
import puppeteer from 'puppeteer-core';
import fs from 'fs';
const CHROME = 'C:/Program Files/Google/Chrome/Application/chrome.exe';
const U = process.env.BASE || 'http://localhost:3000';
const stamp = String(Date.now()).slice(-6);
const env = fs.readFileSync('.env.local', 'utf8');
const SU = (env.match(/NEXT_PUBLIC_SUPABASE_URL=(.*)/) || [])[1].trim().replace(/["\r]/g, '');
const SK = (env.match(/NEXT_PUBLIC_SUPABASE_ANON_KEY=(.*)/) || [])[1].trim().replace(/["\r]/g, '');

const b = await puppeteer.launch({ executablePath: CHROME, headless: 'new', args: ['--no-sandbox'] });
const p = await b.newPage();
p.on('dialog', async (d) => { await d.accept(); });
await p.goto(U + '/login', { waitUntil: 'networkidle2', timeout: 30000 });
await p.type('input[type=email]', 'admin@kidzplayful.app');
await p.type('input[type=password]', 'Kidz!admin2026');
await p.click('button[type=submit]');
await p.waitForFunction(() => location.pathname.startsWith('/pilih-anak'), { timeout: 30000 }).catch(() => {});
const { TOK, UID } = await p.evaluate(async (su, sk) => { const j = await (await fetch(su + '/auth/v1/token?grant_type=password', { method: 'POST', headers: { apikey: sk, 'Content-Type': 'application/json' }, body: JSON.stringify({ email: 'admin@kidzplayful.app', password: 'Kidz!admin2026' }) })).json(); return { TOK: j.access_token, UID: j.user.id }; }, SU, SK);
const post = (path, body, pref) => p.evaluate(async (su, p2, b2, pr, sk, tok) => { const r = await fetch(su + '/rest/v1/' + p2, { method: 'POST', headers: { apikey: sk, Authorization: 'Bearer ' + tok, 'Content-Type': 'application/json', ...(pr ? { Prefer: pr } : {}) }, body: JSON.stringify(b2) }); const t = await r.text(); let j; try { j = JSON.parse(t); } catch { j = t; } return { status: r.status, j }; }, SU, path, body, pref, SK, TOK);
const get = (path) => p.evaluate(async (su, p2, sk, tok) => (await (await fetch(su + '/rest/v1/' + p2, { headers: { apikey: sk, Authorization: 'Bearer ' + tok } })).json()), SU, path, SK, TOK);

const tema = (await post('tema', { nama: 'BonusUji ' + stamp, sampul: '⚡', status: 'disetujui', is_minggu_ini: false }, 'return=representation')).j[0];
// target_detik besar → pasti bonus. mesin dekode 1 soal.
const rP = await post('paket_aset', { tema_id: tema.id, mesin: 'dekode', judul: 'Bonus ' + stamp, area_skill: 'kognitif', usia_min: 4, usia_max: 6, target_detik: 999, sumber: 'manual', status: 'disetujui', butir: { legenda: [{ simbol: '🔴', nilai: 'A' }, { simbol: '🔵', nilai: 'B' }], soal: [['🔴']] } }, 'return=representation');
console.log('0. INSERT paket target_detik (kolom 0033):', rP.status === 201 ? 'ok' : 'GAGAL ' + rP.status + ' ' + JSON.stringify(rP.j).slice(0, 140));
const paketId = Array.isArray(rP.j) ? rP.j[0]?.id : null;
const lahir = new Date(Date.now() - 5 * 365 * 864e5).toISOString().slice(0, 10);
const anak = (await post('anak', { nama: 'AnakBonus ' + stamp, ortu_id: UID, tanggal_lahir: lahir }, 'return=representation')).j[0];
const childId = anak?.id;
const koinAwal = (await get(`anak?select=koin&id=eq.${childId}`))[0]?.koin ?? 0;
console.log('setup:', tema?.id && paketId && childId ? 'ok' : 'GAGAL', '| koin awal:', koinAwal);

async function klik(teks) { const bs = await p.$$('button'); for (const bt of bs) { const t = await p.evaluate((el) => el.textContent.trim(), bt); if (t === teks) { await bt.click(); return true; } } return false; }

await p.goto(`${U}/main/${childId}?paket=${paketId}`, { waitUntil: 'networkidle2', timeout: 30000 });
await p.waitForFunction(() => document.body.innerText.includes('Soal'), { timeout: 20000 }).catch(() => {});
await new Promise((r) => setTimeout(r, 400));
const ans = await klik('A'); await new Promise((r) => setTimeout(r, 900));
const reward = await p.waitForFunction(() => document.body.innerText.includes('Hebat'), { timeout: 15000 }).then(() => true).catch(() => false);
const adaBonus = await p.evaluate(() => document.body.innerText.includes('Bonus'));
console.log('1. MAIN + bonus:', ans ? 'ok' : 'GAGAL', '| Reward:', reward ? 'MUNCUL' : 'GAGAL', '| badge ⚡Bonus:', adaBonus);

await new Promise((r) => setTimeout(r, 2000));
const koinAkhir = (await get(`anak?select=koin&id=eq.${childId}`))[0]?.koin ?? 0;
const hasil = await get(`hasil_main?select=mesin,bintang,durasi_detik&anak_id=eq.${childId}`);
console.log('2. koin:', koinAwal, '→', koinAkhir, '(harap +4 = benar1 + bonus3):', koinAkhir - koinAwal === 4, '| hasil:', JSON.stringify(hasil));

// rapor: waktu per game
await p.goto(`${U}/anak/${childId}/laporan`, { waitUntil: 'networkidle2', timeout: 30000 });
const adaPerGame = await p.evaluate(() => document.body.innerText.includes('WAKTU PER GAME') && document.body.innerText.includes('Pecahkan Kode'));
console.log('3. Rapor "Waktu per game":', adaPerGame ? 'ok' : 'GAGAL');

await p.evaluate(async (su, sk, tok, tid) => { await fetch(`${su}/rest/v1/tema?id=eq.${tid}`, { method: 'DELETE', headers: { apikey: sk, Authorization: 'Bearer ' + tok } }); }, SU, SK, TOK, tema.id);
if (childId) await p.evaluate(async (su, sk, tok, cid) => { await fetch(`${su}/rest/v1/anak?id=eq.${cid}`, { method: 'DELETE', headers: { apikey: sk, Authorization: 'Bearer ' + tok } }); }, SU, SK, TOK, childId);
console.log('4. CLEANUP: selesai');
await b.close();
