// Verifikasi e2e engine 'cocokkan'. Prasyarat: migrasi 0035.
// npm run build && npm start, lalu: node tools/cocokkan_check.mjs
...
const tema = (await post('tema', { nama: 'CocokUji ' + stamp, sampul: '🔗', status: 'disetujui', is_minggu_ini: false }, 'return=representation')).j[0];
const butir = { pasangan: [{ kiri: '🐶', kanan: '🦴' }, { kiri: '🐱', kanan: '🐟' }, { kiri: '🐝', kanan: '🍯' }] };
const rC = await post('paket_aset', { tema_id: tema.id, mesin: 'cocokkan', judul: 'Cocok ' + stamp, area_skill: 'kognitif', usia_min: 4, usia_max: 6, sumber: 'manual', status: 'disetujui', butir }, 'return=representation');
console.log('0. INSERT cocokkan (CHECK 0035):', rC.status === 201 ? 'ok' : 'GAGAL ' + rC.status + ' ' + JSON.stringify(rC.j).slice(0, 140));
const paketId = Array.isArray(rC.j) ? rC.j[0]?.id : null;
const lahir = new Date(Date.now() - 5 * 365 * 864e5).toISOString().slice(0, 10);
const childId = (await post('anak', { nama: 'AnakCocok ' + stamp, ortu_id: UID, tanggal_lahir: lahir }, 'return=representation')).j[0]?.id;
...
await p.waitForFunction(() => document.body.innerText.includes('pasangannya'), { timeout: 20000 }).catch(() => {});
await new Promise((r) => setTimeout(r, 400));
const pasang = [['🐶', '🦴'], ['🐱', '🐟'], ['🐝', '🍯']];
let ok = true;
for (const [ki, ka] of pasang) { const a = await klik(ki); await new Promise((r) => setTimeout(r, 250)); const bb = await klik(ka); await new Promise((r) => setTimeout(r, 400)); ok = ok && a && bb; }
...
console.log('1. MAIN Cocokkan (3 pasang):', ok ? 'ok' : 'GAGAL', '| Reward:', reward ? 'MUNCUL' : 'GAGAL', '|', skor);
...
console.log('2. hasil_main:', JSON.stringify(hasil), '| cocokkan:', hasil.some((h) => h.mesin === 'cocokkan'));
