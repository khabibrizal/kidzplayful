// Verifikasi e2e engine 'garis' (Titik & Garis). Prasyarat: migrasi 0037.
// npm run build && npm start, lalu: node tools/garis_check.mjs
import puppeteer from 'puppeteer-core';
import fs from 'fs';
const CHROME = 'C:/Program Files/Google/Chrome/Application/chrome.exe';
const U = process.env.BASE || 'http://localhost:3000';
const stamp = String(Date.now()).slice(-6);
const env = fs.readFileSync('.env.local', 'utf8');
const SU = (env.match(/NEXT_PUBLIC_SUPABASE_URL=(.*)/) || [])[1].trim().replace(/["\r]/g, '');
const SK = (env.match(/NEXT_PUBLIC_SUPABASE_ANON_KEY=(.*)/) || [])[1].trim().replace(/["\r]/g, '');
const b = await puppeteer.launch({ executablePath: CHROME, headless: 'new', args: ['--no-sandbox'] });
const p = await b.newPage(); p.on('dialog', async (d) => { await d.accept(); });
await p.goto(U + '/login', { waitUntil: 'networkidle2', timeout: 30000 });
await p.type('input[type=email]', 'admin@kidzplayful.app'); await p.type('input[type=password]', 'Kidz!admin2026');
await p.click('button[type=submit]'); await p.waitForFunction(() => location.pathname.startsWith('/pilih-anak'), { timeout: 30000 }).catch(() => {});
const { TOK, UID } = await p.evaluate(async (su, sk) => { const j = await (await fetch(su + '/auth/v1/token?grant_type=password', { method: 'POST', headers: { apikey: sk, 'Content-Type': 'application/json' }, body: JSON.stringify({ email: 'admin@kidzplayful.app', password: 'Kidz!admin2026' }) })).json(); return { TOK: j.access_token, UID: j.user.id }; }, SU, SK);
const post = (path, body, pref) => p.evaluate(async (su, p2, b2, pr, sk, tok) => { const r = await fetch(su + '/rest/v1/' + p2, { method: 'POST', headers: { apikey: sk, Authorization: 'Bearer ' + tok, 'Content-Type': 'application/json', ...(pr ? { Prefer: pr } : {}) }, body: JSON.stringify(b2) }); const t = await r.text(); let j; try { j = JSON.parse(t); } catch { j = t; } return { status: r.status, j }; }, SU, path, body, pref, SK, TOK);

const tema = (await post('tema', { nama: 'GarisUji ' + stamp, sampul: '📐', status: 'disetujui', is_minggu_ini: false }, 'return=representation')).j[0];
// 2 soal grid 2x2: soal1 garis atas [0,1]; soal2 diagonal [0,3]
const butir = { soal: [{ kolom: 2, baris: 2, garis: [[0, 1]] }, { kolom: 2, baris: 2, garis: [[0, 3]] }] };
const rG = await post('paket_aset', { tema_id: tema.id, mesin: 'garis', judul: 'Garis ' + stamp, area_skill: 'motorik-halus', usia_min: 4, usia_max: 6, sumber: 'manual', status: 'disetujui', butir }, 'return=representation');
console.log('0. INSERT garis (CHECK 0037):', rG.status === 201 ? 'ok' : 'GAGAL ' + rG.status + ' ' + JSON.stringify(rG.j).slice(0, 140));
const paketId = Array.isArray(rG.j) ? rG.j[0]?.id : null;
const lahir = new Date(Date.now() - 5 * 365 * 864e5).toISOString().slice(0, 10);
const childId = (await post('anak', { nama: 'AnakGaris ' + stamp, ortu_id: UID, tanggal_lahir: lahir }, 'return=representation')).j[0]?.id;
console.log('setup:', tema?.id && paketId && childId ? 'ok' : 'GAGAL');

// klik titik ke-k pada grid interaktif (svg utama = 4 lingkaran terakhir untuk grid 2x2)
async function tapMain(k) { const cs = await p.$$('circle'); const main = cs.slice(cs.length - 4); if (main[k]) { await main[k].click(); return true; } return false; }

await p.goto(`${U}/main/${childId}?paket=${paketId}`, { waitUntil: 'networkidle2', timeout: 30000 });
await p.waitForSelector('circle', { timeout: 20000 });
await new Promise((r) => setTimeout(r, 500));
// soal1: hubungkan titik 0-1
const s1a = await tapMain(0); await new Promise((r) => setTimeout(r, 300)); const s1b = await tapMain(1); await new Promise((r) => setTimeout(r, 1000));
// soal2: hubungkan titik 0-3
const s2a = await tapMain(0); await new Promise((r) => setTimeout(r, 300)); const s2b = await tapMain(3); await new Promise((r) => setTimeout(r, 1000));
const reward = await p.waitForFunction(() => document.body.innerText.includes('Hebat'), { timeout: 15000 }).then(() => true).catch(() => false);
console.log('1. MAIN Garis (0-1 lalu 0-3):', s1a && s1b && s2a && s2b ? 'ok' : 'GAGAL', '| Reward:', reward ? 'MUNCUL' : 'GAGAL');

await new Promise((r) => setTimeout(r, 2000));
const hasil = await p.evaluate(async (su, sk, tok, cid) => (await (await fetch(`${su}/rest/v1/hasil_main?select=mesin,area_skill,bintang&anak_id=eq.${cid}`, { headers: { apikey: sk, Authorization: 'Bearer ' + tok } })).json()), SU, SK, TOK, childId);
console.log('2. hasil_main:', JSON.stringify(hasil), '| garis:', hasil.some((h) => h.mesin === 'garis'));

await p.evaluate(async (su, sk, tok, tid) => { await fetch(`${su}/rest/v1/tema?id=eq.${tid}`, { method: 'DELETE', headers: { apikey: sk, Authorization: 'Bearer ' + tok } }); }, SU, SK, TOK, tema.id);
if (childId) await p.evaluate(async (su, sk, tok, cid) => { await fetch(`${su}/rest/v1/anak?id=eq.${cid}`, { method: 'DELETE', headers: { apikey: sk, Authorization: 'Bearer ' + tok } }); }, SU, SK, TOK, childId);
console.log('3. CLEANUP: selesai');
await b.close();
